...
const portableTextComponents: Partial<PortableTextReactComponents> = {
  block: {
    normal: ({ children }) => {
      const isEmpty =
        !children ||
        (Array.isArray(children) && children.every((child) => child === "" || child === null || child === undefined))
      if (isEmpty) {
        return <p className="text-base leading-relaxed mb-4 font-sans">&nbsp;</p>
      }
      return <p className="text-base leading-relaxed mb-4 font-sans">{children}</p>
    },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="flex items-center justify-center">
              <div className="prose prose-sm max-w-none text-center">
                {contactData?.content ? (
                  <PortableText value={contactData.content} components={portableTextComponents} />
                ) : (
